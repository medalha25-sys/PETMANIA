...
const Financeiro: React.FC = () => {
    ...
    const [showRegisterClosedWarning, setShowRegisterClosedWarning] = useState(false);
    ...
    // ... (fetchCashRegister, fetchCompanyInfo code unchanged)
    ...
    const openModal = (type: 'income' | 'expense') => {
        // Warning if trying to add income with closed register
        if (type === 'income' && cashRegister?.status !== 'open') {
            setShowRegisterClosedWarning(true);
            return;
        }
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 max-w-[1400px] mx-auto w-full pb-24 lg:pb-8 relative">
            ...
            {/* Warning Modal for Closed Register */}
            {showRegisterClosedWarning && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-800 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700">
                        <div className="p-6 flex flex-col items-center text-center gap-4">
                            ...
                            <div className="grid grid-cols-2 gap-3 w-full mt-2">
                                <button
                                    onClick={() => setShowRegisterClosedWarning(false)}
                                ...
                                <button
                                    onClick={() => {
                                        setShowRegisterClosedWarning(false);
