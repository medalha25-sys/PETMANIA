...
const MobileNav: React.FC<MobileNavProps> = ({ isGuest = false }) => {
    ...
    return (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-surface-dark border-t border-gray-200 dark:border-gray-800 flex justify-around p-3 z-50 safe-area-bottom shadow-lg">
            ...
            {!isGuest ? (
                ...
                <>
                    <Link to="/produtos" className={`flex flex-col items-center gap-1 ${location.pathname === '/produtos' ? 'text-primary' : 'text-slate-400 dark:text-slate-500'}`}>
                        <span className="material-symbols-outlined">shopping_bag</span>
                        <span className="text-[10px] font-medium">Produtos</span>
                    </Link>
                    <Link to="/servicos" className={`flex flex-col items-center gap-1 ${location.pathname === '/servicos' ? 'text-primary' : 'text-slate-400 dark:text-slate-500'}`}>
